...
This document redesigns the [unified heap + self-improvement plan](unified_heap_and_self_improvement.plan.md) around **good and easy UI/UX**, with a core rule: **the user is not confused and has only the interactions that are strictly necessary.** Same features; user-first presentation. **Please review and give feedback.**
...
## 8. Summary: UI/UX checklist

| Feature | Where | What user sees / does | Easy because |
|--------|--------|------------------------|--------------|
| Heap vs standard | Chat (header or settings) | One control: Standard / Multi-agent | One place, clear labels |
| Goal achieved? | Run page (when waiting) | Question + buttons: Yes, Improve and retry, I’ll say what went wrong | One screen, no jargon |
| Give feedback | Run page (inline) | Text area + "Submit feedback and retry" | One field, one click |
| Improve and retry | Run page (and optionally Chat) | One button | One action |
| Self-improvement config | Workflow editor (agent node panel) | Section "Self-improvement": feedback interval dropdown + checkbox "Allow auto improve" | One place, human labels |
| What went wrong | Run page | Optional "What went wrong?" summary + Execution trail + Shell logs | Quick scan + details on demand |
| Run type | Run page, Chat | Badge or line "Multi-agent" / "Standard" | Visible state |
...
End of UI/UX redesign. Backend and implementation order stay as in the [unified plan](unified_heap_and_self_improvement.plan.md); this doc is the user-experience lens with **minimal interactions** as the core principle.
