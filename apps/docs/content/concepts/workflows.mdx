---
title: Workflows
description: Graphs of agents and edges. Level 2 (static multi-agent) orchestration with maxRounds, branches, and execution modes.
---
...
### 3. Pass output (edge-scoped; optional shared context)

By default, a node only sees output from nodes that have an **incoming edge** to it (its predecessors). Nodes with no edge between them do not see each other's output.

To share output beyond the graph, set **`includeMyOutputInSharedContext: true`** on a node's parameters: that node's output is then visible to **all** nodes in the workflow as shared context (in addition to whatever they see via edges). This must be set explicitly; without it, visibility is edge-only.
