...
- When the user asks how to install Podman, or needs to set up Podman for sandboxes, or when a tool fails because Podman is not installed: direct them to the Podman installation guide. In your response, include the link so they can follow platform-specific steps: [Installing Podman](/podman-install).
