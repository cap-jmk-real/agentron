...
export const REMOTE_TOOLS: AssistantToolDef[] = [
  {
    ...
    description: "List saved remote servers used for custom-deployed LLM access (e.g. SSH tunnel to Ollama). Use when the user asks what remote servers are saved or wants to use one for a new agent.",
  ...
  },
  {
    ...
    parameters: {
      ...
      properties: {
        ...
        keyPath: { type: "string", description: "Path to private key file (required for automated test when authType is key)" },
      },
  ...
  },
  {
    ...
    description: "Save a remote server configuration for use with new agents. Call this when the user confirms they want to save after a successful connection test (or after they tested manually). Do not store passwords; for password auth we only save host/port/user and the user will be prompted when using.",
    parameters: {
      type: "object",
      properties: {
        ...
        modelBaseUrl: { type: "string", description: "URL of the model API on the server, e.g. http://127.0.0.1:11434 for Ollama" },
      },
