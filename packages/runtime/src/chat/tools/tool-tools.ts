...
export const TOOL_TOOLS: AssistantToolDef[] = [
    ...
    parameters: {
      ...
      properties: {
        specialistId: {
          type: "string",
          description:
            "Optional: specialist id (e.g. improvement, agent). Omit to get options for all top-level specialists.",
        },
      },
      required: [],
    },
    ...
    parameters: {
      ...
      properties: {
        category: {
          type: "string",
          description:
            "Optional: vault | web | browser | containers | files | user_input | improvement.",
        },
        subset: {
          type: "string",
          description:
            "When category is 'improvement': 'observe' (run/feedback); 'prompt' (prompt-adjustment only); 'topology' (workflow/agent graph only); 'prompt_and_topology' (observe + prompt + topology, no model training); 'training' (jobs and training pipeline). Omit for all improvement tools.",
        },
      },
      required: [],
    },
  },
  {
    name: "get_tool",
    description:
      "Get full details of a tool by ID (name, protocol, config, inputSchema). Use before update_tool when fixing a tool.",
    parameters: {
      type: "object",
  ...
  },
  {
    name: "update_tool",
    description:
      "Update an existing tool. Use when fixing a tool — set name, config, inputSchema, outputSchema. Standard tools (std-*) can only update inputSchema/outputSchema.",
    parameters: {
      type: "object",
      properties: {
        ...
        config: {
          type: "object",
          description: "Tool config (e.g. HTTP: { url, method }, MCP: server config)",
        },
  ...
  },
  {
    name: "create_tool",
    description:
      "Create a new tool and save it to the database. Tools can be native (built-in/code), HTTP (call a URL), or MCP. The tool will appear in the studio sidebar under Tools and can be attached to agents.",
    parameters: {
      type: "object",
      properties: {
        ...
        protocol: {
          type: "string",
          enum: ["native", "mcp", "http"],
          description: "native = built-in/code, http = call URL, mcp = MCP server",
        },
        config: {
          type: "object",
          description: "For HTTP: { url, method }. For MCP: server config. Optional.",
        },
