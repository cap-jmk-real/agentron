...
export const WORKFLOW_TOOLS: AssistantToolDef[] = [
  ...
  },
  {
    name: "cancel_run",
    description:
      "Cancel a workflow run that is waiting for user input. Use when the user says they want to stop, cancel, or abort the run.",
    parameters: {
      type: "object",
      properties: {
    ...
    parameters: {
      ...
      properties: {
        ...
        response: {
          type: "string",
          description:
            "The exact user message (full option label or typed text); never use '1', '2', or similar — pass the full text.",
        },
      },
  ...
  },
  {
    name: "get_run",
    description:
      "Get a run/execution by ID. Returns status, output (with output.output and output.trail), error. Use to inspect what agents actually said and did (trail = per-step input/output). Call after execute_workflow when you need to re-read a run, or when the user refers to a specific run.",
    parameters: {
      type: "object",
      properties: {
        id: { type: "string", description: "Run/execution ID (returned by execute_workflow)" },
      },
      required: ["id"],
    },
  },
  {
    ...
    description:
      "Get the workflow/run message log for a run (persisted agent and user messages). Use to see what has been said in the workflow so far (e.g. when a run is waiting for input or when the user asks what happened in a run). Returns ordered messages (role, content, nodeId, agentId, createdAt).",
    parameters: {
      type: "object",
      properties: {
        ...
        limit: {
          type: "number",
          description: "Optional max number of messages to return (default 50, max 100)",
        },
      },
      required: ["runId"],
    },
  },
  {
    ...
    description:
      "Load a run with bounded context for improving an agent: returns run metadata, trail summary (one line per step), and recent errors from run_logs. Use when improving from a failed or incomplete run (e.g. improvement workflow). First call with runId only; only pass includeFullLogs: true if the summary is insufficient to diagnose or fix the failure.",
    parameters: {
      type: "object",
      properties: {
        ...
        includeFullLogs: {
          type: "boolean",
          description:
            "If true, return full trail and run_logs. Default false. Only set true when the bounded summary is insufficient.",
        },
      },
      required: ["runId"],
    },
  },
  {
    ...
    description:
      "List recent feedback for a target (agent or workflow) so you can improve from past user ratings. Returns short rows (notes, input/output summaries) for the targetId. Use label to filter by good/bad feedback, and limit to cap the number of rows (default 20, max 50).",
    parameters: {
      type: "object",
      properties: {
        targetId: {
          type: "string",
          description: "Target id (agent or workflow) whose feedback you want to load",
        },
  ...
  },
  {
    name: "get_workflow",
    description:
      "Get a workflow by ID including its nodes, edges, and optional branches. Branches are disconnected graphs with their own schedule (interval, daily@HH:mm, weekly@0,1,2). Use when you need to read the current workflow graph or branches before updating.",
    parameters: {
      type: "object",
  ...
  },
  {
    ...
    description:
      "Add one or more edges (and optionally new nodes) to an existing workflow without replacing the existing graph. Use this to connect existing agent nodes or add a new agent and connect it. For new nodes: parameters.agentId MUST be the exact UUID from create_agent, never placeholders. When adding edges that create a loop, pass maxRounds (e.g. 4 or 10) so execution does not run forever.",
    parameters: {
      type: "object",
      properties: {
        ...
        edges: {
          type: "array",
          description: "Canvas format: [{ id, source: nodeId, target: nodeId }]",
        },
        nodes: {
          type: "array",
          description:
            "Optional new nodes: [{ id, type: 'agent', position: [x,y], parameters: { agentId: '<uuid-from-create_agent>' } }]",
        },
        maxRounds: {
          type: "number",
          description:
            "When edges create a loop: number of full cycles. One cycle = each agent in the loop runs once. E.g. 2-agent chat '3 rounds each' → maxRounds: 3 (3 cycles = 6 steps). Persisted.",
        },
        turnInstruction: {
          type: "string",
          description:
            "Optional instruction for each agent turn (e.g. 'Reply directly to what the partner just said.') to get a real back-and-forth. Persisted.",
        },
      },
      required: ["id", "edges"],
    },
    ...
    parameters: {
      ...
      properties: {
        ...
        maxRounds: {
          type: "number",
          description:
            "REQUIRED when edges form a loop. Number of full cycles (one cycle = each agent speaks once). E.g. 2-agent '3 rounds each' → maxRounds: 3.",
        },
        turnInstruction: {
          type: "string",
          description:
            "Optional instruction shown at the start of each agent turn. Omit to leave unset.",
        },
        schedule: {
          type: "string",
          description:
            "Optional top-level schedule: interval seconds (e.g. '60'), daily@HH:mm, or weekly@0,1,2. Used when workflow has no branches.",
        },
        nodes: {
          type: "array",
          description:
            "Each item: { id, type: 'agent', position: [x,y], parameters: { agentId: '<uuid-from-create_agent>' } }. Only agent nodes; do not add type 'tool' — tools are on the agent via toolIds.",
        },
        edges: { type: "array", description: "Each item: { id, source: nodeId, target: nodeId }" },
        branches: {
          type: "array",
          description:
            "Optional array of disconnected graphs. Each branch: { id, name?, nodes, edges, maxRounds?, schedule?, executionMode? }. executionMode: one_time = run only when user triggers; interval = fixed schedule (schedule required: seconds, daily@HH:mm, weekly@0,1,2); continuous = re-run after each run completes (schedule optional = delay in seconds between runs). Branches run in parallel; mix modes freely.",
        },
      },
      required: ["id"],
    },
  },
  {
    ...
    description:
      "Delete a workflow by ID. Use when the user asks to delete or remove a workflow. Get workflow ids from list_workflows.",
    parameters: {
      type: "object",
  ...
  },
  {
    ...
    description:
      "List version history for a workflow (id, version, created_at). Use before rollback_workflow so the user can pick a version.",
    parameters: {
      type: "object",
  ...
  },
  {
    ...
    description:
      "Restore a workflow to a previous version. Use list_workflow_versions first to get version id or version number.",
    parameters: {
      type: "object",
      properties: {
        ...
        version: {
          type: "number",
          description: "Version number (1-based) from list_workflow_versions",
        },
      },
      required: ["workflowId"],
    },
  },
  {
    ...
    parameters: {
      ...
      properties: {
        id: {
          type: "string",
          description: "Workflow ID to run (from create_workflow or get_workflow)",
        },
        branchId: {
          type: "string",
          description:
