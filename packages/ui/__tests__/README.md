...
**Red vs Blue E2E (workflow):** Run only the red-vs-blue test (Attacker vs Defender on a Shellshock target container). Requires Podman or Docker and Ollama. From **repo root**: `npm run test:e2e-llm -- red-vs-blue` (args after `--` are forwarded to Vitest; Vitest runs files whose path contains the pattern). To save a log artifact: **PowerShell** `$env:E2E_SAVE_ARTIFACTS="1"; npm run test:e2e-llm -- red-vs-blue`; **Bash** `E2E_SAVE_ARTIFACTS=1 npm run test:e2e-llm -- red-vs-blue`. Artifact: `packages/ui/__tests__/e2e/artifacts/e2e-red_vs_blue-<timestamp>.log`. **Live progress:** Set `E2E_LOG_PROGRESS=1` (or use `E2E_SAVE_ARTIFACTS=1`) to log workflow progress (trail length, message, last step) to stdout and to a progress log file in `packages/ui/__tests__/e2e/artifacts/` (e.g. `e2e-progress-<workflowId>-<timestamp>.log`) for easier reading; progress is inferred from the DB. **Containers:** The test uses the app default engine (Podman unless you set Docker in Settings). To list them: `podman ps -a` or `docker ps -a`; use `--filter label=agentos=true` to see only Agentron-created containers.
