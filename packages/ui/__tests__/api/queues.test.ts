...
describe("Queues API", () => {
  ...
  it("GET /api/queues returns inserted conversation locks", async () => {
    ...
    try {
      ...
      expect(res.status).toBe(200);
      const data = await res.json();
      const found = data.conversationLocks.find((l: { conversationId: string }) => l.conversationId === convId);
  ...
  it("GET /api/queues returns activeChatTraces for locked conversations with assistant trace data", async () => {
    ...
    await db
      ...
      .values({
        ...
        llmTrace: JSON.stringify([{ phase: "llm_response", messageCount: 3, responsePreview: "Here is the run." }]),
        createdAt: now - 100,
      })
      .run();
    try {
      ...
      const trace = data.activeChatTraces.find((t: { conversationId: string }) => t.conversationId === convId);
  ...
  it("GET /api/queues returns messageQueueLog steps for locked conversations", async () => {
    ...
    await db.insert(messageQueueLog).values({ id: id1, conversationId: convId, messageId: null, type: "trace_step", phase: "prepare", label: "Preparing context…", payload: null, createdAt: now - 200 }).run();
    await db.insert(messageQueueLog).values({ id: id2, conversationId: convId, messageId: null, type: "trace_step", phase: "rephrase", label: "Rephrasing…", payload: null, createdAt: now - 100 }).run();
    try {
      const res = await GET();
      expect(res.status).toBe(200);
      const data = await res.json();
      ...
      const entry = data.messageQueueLog.find((e: { conversationId: string }) => e.conversationId === convId);
...
describe("Queues message-log API", () => {
  it("GET /api/queues/message-log without conversationId returns conversations list and nextOffset", async () => {
    ...
    data.conversations.forEach((c: { conversationId: string; lastAt: number; stepCount: number }) => {
  ...
  it("GET /api/queues/message-log with conversationId returns steps and nextCursor", async () => {
    ...
    await db.insert(messageQueueLog).values({ id: id1, conversationId: convId, messageId: null, type: "trace_step", phase: "prepare", label: "Preparing…", payload: null, createdAt: now }).run();
    await db.insert(messageQueueLog).values({ id: id2, conversationId: convId, messageId: null, type: "trace_step", phase: "rephrase", label: "Rephrasing…", payload: JSON.stringify({ inputPreview: "hi" }), createdAt: now + 10 }).run();
    try {
  ...
  it("GET /api/queues/message-log with conversationId and limit paginates steps", async () => {
    ...
    for (let i = 0; i < 4; i++) {
      await db.insert(messageQueueLog).values({ id: ids[i], conversationId: convId, messageId: null, type: "trace_step", phase: "step", label: `Step ${i}`, payload: null, createdAt: now + i }).run();
    }
    try {
      const res = await getMessageLog(new Request(`http://localhost/api/queues/message-log?conversationId=${encodeURIComponent(convId)}&limit=2`));
      expect(res.status).toBe(200);
      const data = await res.json();
      expect(data.steps).toHaveLength(2);
      ...
      const res2 = await getMessageLog(new Request(`http://localhost/api/queues/message-log?conversationId=${encodeURIComponent(convId)}&limit=2&cursor=${encodeURIComponent(cursor)}`));
  ...
  it("GET /api/queues/message-log without conversationId returns paginated conversations with nextOffset", async () => {
    ...
    const now = Date.now();
    await db.insert(messageQueueLog).values({ id: "c1-" + now, conversationId: convId, messageId: null, type: "trace_step", phase: "x", label: "X", payload: null, createdAt: now }).run();
    try {
      const res = await getMessageLog(new Request("http://localhost/api/queues/message-log?limit=20&offset=0"));
      expect(res.status).toBe(200);
      const data = await res.json();
      expect(Array.isArray(data.conversations)).toBe(true);
      const found = data.conversations.find((c: { conversationId: string }) => c.conversationId === convId);
      expect(found).toBeDefined();
  ...
  it("GET /api/queues/message-log with unknown conversationId returns empty steps", async () => {
    const res = await getMessageLog(new Request("http://localhost/api/queues/message-log?conversationId=no-such-conversation-id-12345"));
    expect(res.status).toBe(200);
    const data = await res.json();
