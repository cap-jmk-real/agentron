...
describe("GET /api/rag/connectors/:id/items (browse)", () => {
  ...
  it("returns at most limit items and nextPageToken when more exist", async () => {
    const tmpDir = path.resolve(path.join(os.tmpdir(), `rag-browse-limit-${Date.now()}`));
    fs.mkdirSync(tmpDir, { recursive: true });
    fs.writeFileSync(path.join(tmpDir, "1.txt"), "1");
    fs.writeFileSync(path.join(tmpDir, "2.txt"), "2");
    fs.writeFileSync(path.join(tmpDir, "3.txt"), "3");
    try {
      const encRes = await encPost(
        new Request("http://localhost/api/rag/encoding-config", {
          method: "POST",
      ...
      const getRes = await GET(
        new Request(`http://localhost/api/rag/connectors/${id}/items?limit=2`),
        { params: Promise.resolve({ id }) }
      );
      expect(getRes.status).toBe(200);
      const data = await getRes.json();
      ...
      const page2Res = await GET(
        new Request(
          `http://localhost/api/rag/connectors/${id}/items?limit=10&pageToken=${encodeURIComponent(data.nextPageToken)}`
        ),
        { params: Promise.resolve({ id }) }
      );
  ...
  it("returns 400 when path is not a directory (browse throws)", async () => {
    const tmpDir = path.resolve(path.join(os.tmpdir(), `rag-browse-nodir-${Date.now()}`));
    fs.writeFileSync(tmpDir, "file not dir");
    try {
      const encRes = await encPost(
        new Request("http://localhost/api/rag/encoding-config", {
          method: "POST",
  ...
  it("returns 400 for connector type with no browse implementation", async () => {
    const encRes = await encPost(
      new Request("http://localhost/api/rag/encoding-config", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    expect(data.error).toContain("unsupported_browse_type");
  });
});
