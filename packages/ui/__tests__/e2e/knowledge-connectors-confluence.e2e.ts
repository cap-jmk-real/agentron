/**
 * E2E: Knowledge connectors (Confluence) — API flow when E2E_CONFLUENCE_AUTH=1.
 * When set: creates collection, adds confluence connector, calls sync; passes on 200 or 400.
...
const E2E_CONFLUENCE_AUTH = process.env.E2E_CONFLUENCE_AUTH === "1";

describe("e2e knowledge-connectors-confluence", () => {
  it("when E2E_CONFLUENCE_AUTH unset, test is skipped", () => {
    if (!E2E_CONFLUENCE_AUTH) {
      expect(E2E_CONFLUENCE_AUTH).toBe(false);
      return;
    }
  });

  it("when E2E_CONFLUENCE_AUTH=1, create connector and sync (200 or 400 error)", async () => {
    if (!E2E_CONFLUENCE_AUTH) return;
    ...
    const provRes = await embeddingProviderPost(
      new Request("http://localhost/api/rag/embedding-providers", {
        ...
        body: JSON.stringify({
          name: "E2E Confluence local embed",
          type: "local",
    ...
    if (syncRes.status === 200) {
      ...
      e2eLog.step("confluence_sync_ok", { connectorId });
    } else {
      expect(syncRes.status).toBe(400);
      expect(data.error).toBeDefined();
      e2eLog.step("confluence_sync_error", { error: (data.error ?? "").slice(0, 80) });
    }
  });
});
