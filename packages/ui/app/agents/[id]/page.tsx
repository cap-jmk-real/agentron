...
export default function AgentDetailPage() {
  ...
  const save = async (overrideGraph?: { nodes: unknown[]; edges: unknown[] }) => {
    ...
    if (overrideGraph) {
      defToSave = {
        ...definition,
        graph: { nodes: overrideGraph.nodes, edges: overrideGraph.edges },
      };
    } else {
      const graph = definition.graph as { nodes?: unknown[]; edges?: unknown[] } | undefined;
      const hasGraphInMemory =
  ...
  if (!agentId || !agent) {
    return (
      <div className="card card-narrow">
        ...
        <p className="card-narrow-desc">The agent may have been deleted or the link is invalid.</p>
          ...
          <div style={{ display: "grid", gap: "0.75rem", gridTemplateColumns: "1fr 1fr 1fr" }}>
            ...
            <div className="field">
              ...
              <input
                ...
                placeholder="analysis, generation"
              />
            </div>
          </div>
        </div>
      ...
      <div className="tab-content">
        {activeTab === "prompts" && (
          <PromptsEditor
            ...
            onDefinitionChange={(def) => setDefinition(def as AgentDefinition)}
          />
        )}
        {activeTab === "tools" && (
          <ToolsEditor
        ...
        {activeTab === "knowledge" && (
          <div className="card">
            ...
            <p style={{ color: "var(--text-muted)", fontSize: "0.88rem", margin: "0 0 1rem" }}>
              Use studio knowledge (deployment collection for chat) or a custom collection for this
              agent.
            </p>
            <div className="field">
            ...
            <p style={{ fontSize: "0.82rem", color: "var(--text-muted)", margin: "0.5rem 0 0" }}>
              Manage collections under{" "}
              <Link href="/knowledge" style={{ color: "var(--link)" }}>
                Knowledge
              </Link>{" "}
              in the sidebar. Collections use encoding configs from Knowledge; configure embedding
              endpoints in Settings → Embedding.
            </p>
          </div>
        )}
        {activeTab === "feedback" && (
            ...
            <div className="card canvas-card" style={{ padding: 0 }}>
              <div className="canvas-card-header">
                ...
                <p className="canvas-card-header-desc">
                  Add tools (LLM, Input, Output, Decision, Context, and library tools). Connect them
                  to define execution flow. Customize each tool without changing the library.
                </p>
              </div>
              <AgentCanvas
                nodes={(() => {
                  try {
                    ...
                    const g = definition.graph as
                      | { nodes?: unknown[]; edges?: unknown[] }
                      | undefined;
                    return (Array.isArray(n) ? n : (g?.nodes ?? [])) as {
                      id: string;
                      type: string;
                ...
                edges={(() => {
                  try {
                    ...
                    const g = definition.graph as
                      | { nodes?: unknown[]; edges?: unknown[] }
                      | undefined;
                    return (Array.isArray(e) ? e : (g?.edges ?? [])) as {
                      id: string;
                      source: string;
                      target: string;
                    }[];
                ...
                onSaveToolToLibrary={async (nodeId, baseToolId, override) => {
                  ...
                  const graph = definition.graph as
                    | { nodes?: unknown[]; edges?: unknown[] }
                    | undefined;
                  const nextNodes = (
                    (graph?.nodes ?? []) as {
                  ...
                  const nextEdges =
                    (definition.graph as { nodes?: unknown[]; edges?: unknown[] } | undefined)
                      ?.edges ?? [];
                  setDefinition({
                    ...definition,
                    ...
                    <textarea
                      ...
                      onBlur={() => {
                        try {
                          ...
                          const edges =
                            (definition.graph as { edges?: unknown[] } | undefined)?.edges ?? [];
                          if (Array.isArray(parsed)) {
                            setDefinition({
                              ...definition,
                              graph: { nodes: parsed, edges: Array.isArray(edges) ? edges : [] },
                            });
                          }
                        } catch {
                          /* ignore invalid JSON */
                    ...
                    <textarea
                      ...
                      onBlur={() => {
                        try {
                          ...
                          const nodes =
                            (definition.graph as { nodes?: unknown[] } | undefined)?.nodes ?? [];
                          if (Array.isArray(parsed)) {
                            setDefinition({
                              ...definition,
                              graph: { nodes: Array.isArray(nodes) ? nodes : [], edges: parsed },
                            });
                          }
                        } catch {
                          /* ignore invalid JSON */
        ...
        {activeTab === "permissions" && (
          <div className="card">
            ...
            <p className="text-muted" style={{ fontSize: "0.82rem", margin: 0 }}>
              Coming soon — scope-based access control for tools, context keys, and external
              endpoints.
            </p>
          </div>
        )}
