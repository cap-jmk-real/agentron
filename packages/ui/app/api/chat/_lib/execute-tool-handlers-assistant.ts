...
import { logToolPhase, logToolSuccessAndReturn } from "./execute-tool-shared";
import {
  db,
  assistantMemory,
...
const EXPLAIN_DOCS: Record<string, string> = {
  general:
...
export async function handleExplainSoftware(
  a: Record<string, unknown>,
  _ctx: ExecuteToolContext | undefined
): Promise<unknown> {
  const topic = typeof a.topic === "string" ? a.topic.trim().toLowerCase() || "general" : "general";
  const explanation = EXPLAIN_DOCS[topic] || EXPLAIN_DOCS.general;
...
export async function handleRemember(
  ...
  const value = typeof a.value === "string" ? a.value.trim() : null;
...
export async function handleGetAssistantSetting(
  a: Record<string, unknown>,
  _ctx: ExecuteToolContext | undefined
): Promise<unknown> {
...
export async function handleSetAssistantSetting(
  a: Record<string, unknown>,
  _ctx: ExecuteToolContext | undefined
): Promise<unknown> {
  const key = a.key as string;
  ...
  if (!Number.isFinite(value)) {
    value = DEFAULT_RECENT_SUMMARIES_COUNT;
  } else {
    value = Math.round(value);
    value = Math.max(MIN_SUMMARIES, Math.min(MAX_SUMMARIES, value));
  }
  const now = Date.now();
  await db
    ...
    .onConflictDoUpdate({
      target: chatAssistantSettings.id,
  ...
  try {
    ...
    switch (name) {
      case "answer_question":
        result = await handleAnswerQuestion(a, ctx);
        break;
      case "explain_software":
        result = await handleExplainSoftware(a, ctx);
        break;
      case "remember":
        result = await handleRemember(a, ctx);
        break;
      case "get_assistant_setting":
        result = await handleGetAssistantSetting(a, ctx);
        break;
      case "set_assistant_setting":
        result = await handleSetAssistantSetting(a, ctx);
        break;
      default:
        return undefined;
    }
    return logToolSuccessAndReturn(ctx, name, result);
  } catch (err) {
