    ...
    case "openclaw_history": {
      ...
      function normalizeOpenClawHistoryMessage(msg: Record<string, unknown>): {
        ...
        if (typeof c === "string") {
          ...
          // Full content: text + thinking (interesting); exclude only toolCall.
          ...
          // Real reply: type "text" parts first; then toolCall "speech" (e.g. tts with arguments.text).
          ...
          // Fallback: if no typed text parts (e.g. gateway omits type), use full content as reply.
              ...
              const ttsTexts = partList
                .filter(
                  (p) =>
                    ...
                    ((p as { arguments: { text: string } }).arguments.text as string).trim()
                      .length > 0
                )
                .map((p) =>
                  ((p as { arguments: { text: string } }).arguments.text as string).trim()
        ...
        if (content === "" && Array.isArray(msg.parts))
          content = (msg.parts as Record<string, unknown>[])
            ...
            .join("");
        // Fallback: any string field on the message that looks like body text
        ...
        // Fallback: recursively extract text from content object
        if (
        ...
        // When gateway stores a failed run (e.g. Ollama "Connection error"), content is [] and errorMessage is set.
