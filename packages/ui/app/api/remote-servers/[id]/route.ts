...
export async function GET(_: Request, { params }: Params) {
  const rows = await db.select().from(remoteServers).where(eq(remoteServers.id, params.id));
...
export async function PATCH(request: Request, { params }: Params) {
  ...
  await db.update(remoteServers).set(toRemoteServerRow(updated)).where(eq(remoteServers.id, params.id)).run();
  return json(updated);
}

export async function DELETE(_: Request, { params }: Params) {
  await db.delete(remoteServers).where(eq(remoteServers.id, params.id)).run();
