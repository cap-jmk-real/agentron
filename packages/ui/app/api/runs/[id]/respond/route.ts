...
export async function POST(request: Request, { params }: Params) {
  ...
  if (run.status !== "waiting_for_user") {
    ...
    fetch("http://127.0.0.1:7242/ingest/3176dc2d-c7b9-4633-bc70-1216077b8573", {
      ...
      body: JSON.stringify({
        location: "runs/[id]/respond/route.ts",
        message: "respond rejected run not waiting",
    ...
    // #endregion
    return json(
      { error: "Run is not waiting for user input", status: run.status },
      { status: 400 }
    );
  }
  ...
  fetch("http://127.0.0.1:7242/ingest/3176dc2d-c7b9-4633-bc70-1216077b8573", {
    ...
    body: JSON.stringify({
      location: "runs/[id]/respond/route.ts",
      message: "respond API enqueueing resume",
      data: { runId, responseLen: response.length, hasVaultKey: !!vaultKey },
      hypothesisId: "vault_access",
      timestamp: Date.now(),
    }),
  }).catch(() => {});
