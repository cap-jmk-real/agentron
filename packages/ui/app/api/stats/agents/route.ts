...
export async function GET() {
  ...
  const chatCost = chatRows.reduce((s, r) => s + (r.estimatedCost ? parseFloat(r.estimatedCost) : 0), 0);
  ...
  const totals = {
    ...
    estimatedCost: Math.round(allUsage.reduce((s, r) => s + (r.estimatedCost ? parseFloat(r.estimatedCost) : 0), 0) * 1_000_000) / 1_000_000,
  };

  return json({
    agents: agentStats.sort((a, b) => b.totalTokens - a.totalTokens),
    chat: { totalRuns: chatRows.length, promptTokens: chatPrompt, completionTokens: chatCompletion, totalTokens: chatPrompt + chatCompletion, estimatedCost: Math.round(chatCost * 1_000_000) / 1_000_000 },
    totals,
  });
}
