...
export type AggregateBy = "none" | "source" | "workflow" | "agent";

export function formatRequestQueueTs(ts: number): string {
  const d = new Date(ts);
  return (
...
export function describeContext(ctx: LLMRequestContext): string {
...
export function groupDelayed(
  delayed: DelayedEntry[],
  by: AggregateBy
): Map<string, DelayedEntry[]> {
...
const REQUEST_QUEUE_POLL_MS = 2000;

/** Request queue (rate-limit pending/delayed) section for the Queues page. */
export default function RequestQueueSection() {
  const [data, setData] = useState<RequestQueueData | null>(null);
  ...
  useEffect(() => {
    ...
    fetchQueue();
    const interval = setInterval(fetchQueue, REQUEST_QUEUE_POLL_MS);
            ...
            {pending.map((p) => (
              ...
              >
                <span style={{ color: "var(--text-muted)", marginRight: "0.5rem" }}>
                  {formatRequestQueueTs(p.addedAt)}
                </span>
              ...
              <div key={key}>
                ...
                <ul style={{ listStyle: "none", padding: 0, margin: 0 }}>
                  {entries.map((p) => (
                    ...
                    >
                      {formatRequestQueueTs(p.addedAt)} — {describeContext(p.context)} | {p.key}
                    </li>
                  ))}
                </ul>
              </div>
          ...
          <ul style={{ listStyle: "none", padding: 0, margin: 0 }}>
            {[...recentDelayed]
              ...
              .map((d, i) => (
                ...
                >
                  <span style={{ color: "var(--text-muted)", marginRight: "0.5rem" }}>
                    {formatRequestQueueTs(d.completedAt)}
                  </span>
                ...
                <ul style={{ listStyle: "none", padding: 0, margin: 0 }}>
                  {entries
                    ...
                    .map((d, i) => (
                      ...
                      >
                        {formatRequestQueueTs(d.completedAt)} — {describeContext(d.context)} —
                        waited {d.waitedMs}ms | {d.key}
                      </li>
                    ))}
                </ul>
              </div>
