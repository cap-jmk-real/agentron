...
function Bar({ value, max, color }: { value: number; max: number; color: string }) {
  ...
  return (
    <div style={{ width: "100%", height: 6, borderRadius: 3, background: "var(--border)", overflow: "hidden" }}>
      <div style={{ width: `${pct}%`, height: "100%", borderRadius: 3, background: color, transition: "width 300ms ease" }} />
    </div>
  );
}
...
const CHART_COLORS = { cpu: "var(--primary)", ram: "#6366f1", gpu: "#22c55e", vram: "#14b8a6", disk: "#f59e0b" };

function Sparkline({ data, color }: { data: number[]; color: string }) {
  if (data.length < 2) return <div style={{ height: CHART_HEIGHT, background: "var(--surface-muted)", borderRadius: 4 }} />;
  const min = Math.min(...data);
  const range = (Math.max(...data) - min) || 1;
  const w = 100 / (data.length - 1);
  const points = data.map((v, i) => `${(i * w).toFixed(2)},${(CHART_HEIGHT - ((v - min) / range) * CHART_HEIGHT).toFixed(2)}`).join(" ");
  return (
    <svg viewBox={`0 0 100 ${CHART_HEIGHT}`} width="100%" height={CHART_HEIGHT} preserveAspectRatio="none" style={{ display: "block", borderRadius: 4, overflow: "hidden" }}>
      <polyline fill="none" stroke={color} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" points={points} vectorEffect="non-scaling-stroke" />
    </svg>
  );
}
...
export default function StatsPage() {
  ...
  if (loading) return <div style={{ padding: "2rem", color: "var(--text-muted)" }}>Loading statistics...</div>;

  const maxTokens = Math.max(...agentStats.map((a) => a.totalTokens), 1);

  const ramPct = resourceHistory.map((s) => (s.ram.total > 0 ? (s.ram.used / s.ram.total) * 100 : 0));
  const cpuPct: number[] = [];
  ...
  const diskUsedPct = latest && latest.disk.total > 0 ? Math.round((1 - latest.disk.free / latest.disk.total) * 100) : 0;
  ...
  return (
    <div style={{ maxWidth: 800 }}>
      ...
      <p style={{ fontSize: "0.82rem", color: "var(--text-muted)", margin: "0 0 0.75rem" }}>
        {activeTab === "usage" ? "Token usage and cost breakdown." : "Live resource usage over time (sidebar polls feed this)."}
      </p>
      ...
      {activeTab === "resources" && (
        <div className="card" style={{ padding: "1rem", marginBottom: "1.5rem" }}>
          ...
          <p style={{ fontSize: "0.8rem", color: "var(--text-muted)", margin: "0 0 0.5rem" }}>
            Last ~2.5 minutes. Polling uses the same interval as the sidebar (Settings → System stats refresh).
          </p>
          <div style={{ display: "flex", alignItems: "center", gap: "0.75rem", flexWrap: "wrap", marginBottom: "1rem" }}>
            <label style={{ fontSize: "0.8rem", color: "var(--text-muted)" }}>Refresh interval</label>
            <input
              type="range"
            ...
            />
            <span style={{ fontSize: "0.85rem", fontWeight: 600, minWidth: 52 }}>{formatSystemStatsInterval(statsIntervalMs)}</span>
          </div>
          <div style={{ display: "grid", gap: "1rem" }}>
            <div>
              <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "0.35rem", fontSize: "0.8rem" }}>
                <Cpu size={14} style={{ color: usageBarColor(currentCpu) }} /> <span>CPU (process %)</span>
                <span style={{ marginLeft: "auto", fontWeight: 600, color: usageBarColor(currentCpu) }}>{Math.round(currentCpu)}%</span>
              </div>
              <ResourceBar percent={currentCpu} height={8} />
              <div style={{ marginTop: "0.35rem" }}><Sparkline data={cpuPct} color={CHART_COLORS.cpu} /></div>
            </div>
            <div>
              <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "0.35rem", fontSize: "0.8rem" }}>
                <MemoryStick size={14} style={{ color: usageBarColor(currentRam) }} /> <span>RAM (used %)</span>
                <span style={{ marginLeft: "auto", fontWeight: 600, color: usageBarColor(currentRam) }}>{Math.round(currentRam)}%</span>
              </div>
              <ResourceBar percent={currentRam} height={8} />
              <div style={{ marginTop: "0.35rem" }}><Sparkline data={ramPct} color={CHART_COLORS.ram} /></div>
            </div>
            {gpuPct.some((v) => v > 0) && (
              <>
                <div>
                  <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "0.35rem", fontSize: "0.8rem" }}>
                    <Gauge size={14} style={{ color: usageBarColor(currentGpu) }} /> <span>GPU utilization %</span>
                    <span style={{ marginLeft: "auto", fontWeight: 600, color: usageBarColor(currentGpu) }}>{Math.round(currentGpu)}%</span>
                  </div>
                  <ResourceBar percent={currentGpu} height={8} />
                  <div style={{ marginTop: "0.35rem" }}><Sparkline data={gpuPct} color={CHART_COLORS.gpu} /></div>
                </div>
                <div>
                  <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "0.35rem", fontSize: "0.8rem" }}>
                    <MemoryStick size={14} style={{ color: usageBarColor(currentVram) }} /> <span>VRAM used %</span>
                    <span style={{ marginLeft: "auto", fontWeight: 600, color: usageBarColor(currentVram) }}>{Math.round(currentVram)}%</span>
                  </div>
                  <ResourceBar percent={currentVram} height={8} />
                  <div style={{ marginTop: "0.35rem" }}><Sparkline data={vramPct} color={CHART_COLORS.vram} /></div>
                </div>
              </>
            )}
            <div>
              <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "0.35rem", fontSize: "0.8rem" }}>
                <HardDrive size={14} style={{ color: usageBarColor(diskUsedPct) }} /> <span>Disk used % — {diskFreeNow} GB free</span>
                <span style={{ marginLeft: "auto", fontWeight: 600, color: usageBarColor(diskUsedPct) }}>{diskUsedPct}%</span>
              </div>
              <ResourceBar percent={diskUsedPct} height={8} />
              <div style={{ marginTop: "0.35rem" }}><Sparkline data={diskFreeGb} color={CHART_COLORS.disk} /></div>
            </div>
          </div>
        </div>
      )}
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "0.6rem", marginBottom: "1.5rem" }}>
          {[
            ...
            <div key={card.label} className="card" style={{ padding: "0.75rem 0.85rem" }}>
              <div style={{ fontSize: "0.72rem", color: "var(--text-muted)", fontWeight: 500, marginBottom: "0.2rem" }}>{card.label}</div>
      ...
      <div style={{ marginBottom: "1.5rem" }}>
        ...
        {agentStats.length === 0 ? (
          <p style={{ fontSize: "0.82rem", color: "var(--text-muted)" }}>No agent usage recorded yet.</p>
        ) : (
            ...
            {agentStats.map((a) => (
              <Link key={a.id} href={`/stats/agents/${a.id}`} style={{ textDecoration: "none", color: "inherit" }}>
                <div className="card" style={{ padding: "0.65rem 0.85rem", cursor: "pointer" }}>
                  ...
                  <div style={{ display: "flex", gap: "1rem", marginTop: "0.3rem", fontSize: "0.72rem", color: "var(--text-muted)" }}>
        ...
        <div style={{ marginBottom: "1.5rem" }}>
          ...
          <div className="card" style={{ padding: "0.65rem 0.85rem" }}>
            <div style={{ display: "flex", gap: "1.5rem", fontSize: "0.82rem" }}>
              <span><strong>{fmt(chat.totalRuns)}</strong> calls</span>
              <span><strong>{fmt(chat.totalTokens)}</strong> tokens</span>
              <span><strong>{fmtCost(chat.estimatedCost)}</strong></span>
            </div>
          </div>
        </div>
      )}
      ...
      <div>
        ...
        {workflowStats.length === 0 ? (
          <p style={{ fontSize: "0.82rem", color: "var(--text-muted)" }}>No workflow usage recorded yet.</p>
        ) : (
          <div style={{ display: "grid", gap: "0.4rem" }}>
            {workflowStats.map((wf) => (
              <Link key={wf.id} href={`/stats/workflows/${wf.id}`} style={{ textDecoration: "none", color: "inherit" }}>
                <div className="card" style={{ padding: "0.65rem 0.85rem", cursor: "pointer" }}>
