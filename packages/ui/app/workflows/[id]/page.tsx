...
function parseCalendarSchedule(schedule: string): {
  ...
  if (s.startsWith("weekly@")) {
    ...
    out.weeklyDays = part
      ? part.split(",").map((d) => Math.max(0, Math.min(6, parseInt(d, 10) || 0)))
      : [1];
  }
  return out;
}
...
export default function WorkflowDetailPage() {
  ...
  const hasUnwiredAgentNodes = parsedNodes.some(
    (n) =>
      n.type === "agent" &&
      !(typeof n.parameters?.agentId === "string" && n.parameters.agentId.trim() !== "")
  );
  ...
  const addAgentNode = useCallback(() => {
    ...
    const parsed = [
      ...parsedNodes,
      { id, type: "agent", position: [pos.x, pos.y], parameters: { agentId } },
    ];
      ...
      >
        <button
          ...
          style={{
            ...
            border: "none",
            borderBottom:
              detailTab === "editor" ? "2px solid var(--primary)" : "2px solid transparent",
            cursor: "pointer",
            fontSize: "0.9rem",
            fontWeight: 500,
            display: "inline-flex",
        ...
        <button
          ...
          style={{
            ...
            border: "none",
            borderBottom:
              detailTab === "traces" ? "2px solid var(--primary)" : "2px solid transparent",
            cursor: "pointer",
            fontSize: "0.9rem",
            fontWeight: 500,
            display: "inline-flex",
        ...
        <>
          <div className="card form" style={{ marginBottom: "1.5rem" }}>
                    ...
                    {intervalPreset === "custom" && (
                      ...
                      >
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setCustomIntervalValue(Math.max(1, parseInt(e.target.value, 10) || 1))
                          }
                        />
                        <select
                          className="select"
                          ...
                          onChange={(e) =>
                            setCustomIntervalUnit(e.target.value as "minutes" | "hours")
                          }
                        >
                          ...
                          <input
                            ...
                            onChange={() => {
                              if (calendarWeeklyDays.includes(i)) {
                                ...
                                setCalendarWeeklyDays(
                                  [...calendarWeeklyDays, i].sort((a, b) => a - b)
                                );
                              }
                            }}
                          />
            ...
            <div className="field">
              ...
              />
              <p
                style={{ fontSize: "0.8rem", color: "var(--text-muted)", margin: "0.25rem 0 0 0" }}
              >
                When nodes are connected in a circle (e.g. Agent A → Agent B → Agent A), set this to
                avoid endless loops. Leave empty for linear workflows.
              </p>
            </div>
            <div className="field">
              ...
              <p
                style={{ fontSize: "0.8rem", color: "var(--text-muted)", margin: "0.25rem 0 0 0" }}
              >
                Optional. Shown at the start of each agent turn so they reply to each other instead
                of monologuing. Leave empty for no instruction.
              </p>
            </div>
          </div>
          <div
            className="card form form-wide canvas-card"
          ...
          >
            ...
            >
              ...
              <p style={{ fontSize: "0.85rem", color: "var(--text-muted)", margin: 0 }}>
                Drag agent nodes onto the canvas and connect them. Use tools on each agent to handle
                input and output. Pan and zoom with the controls. Set <strong>Max rounds</strong>{" "}
                above for circular workflows.
              </p>
            </div>
            <WorkflowCanvas
              wfNodes={parsedNodes}
          ...
          <div className="card form form-wide">
            <button
              ...
              style={{
                ...
                alignItems: "center",
                gap: "0.35rem",
                marginBottom: showJson ? "0.75rem" : 0,
                background: "none",
                border: "none",
                cursor: "pointer",
                fontSize: "0.9rem",
